...
const ControllersModule = {
    ...
    async decodeAdvancedFirmware(device) {
        try {
            ...
            // رفع باگ پتانسیل کرش: استخراج نام محصول به صورت محافظت شده
            const productName = device.productName || "Standard Gamepad";
            const vendorName = this.Vendors[device.vendorId] || "سازنده سازگار (Generic/Hall-Effect)";
            updateUI('hw-model', `${productName} (${vendorName})`);
            ...
            if (device.vendorId === 0x054C) {
                ...
                try {
                    ...
                    if (view20 && view20.byteLength >= 10) {
                        // رفع باگ بیتی: استفاده از >>> 0 برای جلوگیری از تولید هگز منفی در متون کلاینت
                        const fwVersion = (((view20.getUint8(2) << 24) | (view20.getUint8(3) << 16) | (view20.getUint8(4) << 8) | view20.getUint8(5)) >>> 0).toString(16).toUpperCase();
                        const sblVersion = (((view20.getUint8(6) << 24) | (view20.getUint8(7) << 16) | (view20.getUint8(8) << 8) | view20.getUint8(9)) >>> 0).toString(16).toUpperCase();
                ...
                try {
                    ...
                    if (view09 && view09.byteLength >= 7) {
                        ...
                        // رفع باگ با به کارگیری متغیر محلی و امن productName
                        const boardRevision = view09.getUint8(0);
                        if (boardRevision === 0x01 || productName.includes("BDM-010")) {
                            updateUI('hw-board-model', "BDM-010 (نسل اول پتانسیومتر)");
                        } else if (boardRevision === 0x02 || productName.includes("BDM-020")) {
